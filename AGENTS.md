...
### 2026-06-08 / 仓库改名与首页整理

- GitHub 仓库已从 `tihu/tihu-hugo-blog` 改名为 `tihu/tihuke`
- 当前远程仓库：`origin = git@github.com:tihu/tihuke.git`
- 当前线上地址：`https://tihu.github.io/tihuke/`
- 旧地址 `https://tihu.github.io/tihu-hugo-blog/` 已确认返回 `404`
- `hugo.toml` 的 `baseURL` 已同步改为 `https://tihu.github.io/tihuke/`
- 本地目录名暂未改，仍为 `/Users/quintinwang/Documents/HugoBlogs/tihu-hugo-blog`；这不影响 Git 远程和线上地址
- 删除首页「往年今日」模块：移除首页 HTML、`.on-this-day` 样式和前端读取 `static/data/on-this-day*.json` 的脚本；数据文件保留
- 已发布两篇新书盘点文章：
  - `content/posts/2026/2026-06-02-books.md`
  - `content/posts/2026/2026-06-08-books.md`
- 本地已通过 `hugo --minify` 构建验证；Hugo 由 Homebrew 安装，当前本机版本为 `0.162.1`
- 今日关键提交：
  - `c04066c`：移除首页往年今日模块
  - `8aa760f`：更新博客仓库地址
  - `5a864d1`：发布六月新书盘点
- 工作区仍有一个未处理的无关变更：`content/posts/.DS_Store`，后续提交时不要误加
